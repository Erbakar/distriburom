...
import { SlidersHorizontal, Heart, Search, Eye } from 'lucide-react';
...
const Products: React.FC = () => {
  ...
  const filteredProducts = PRODUCTS.filter(p => {
    const matchesCat = activeCategory === 'Hepsi' || p.category === activeCategory;
    const matchesSearch = p.name.toLowerCase().includes(searchQuery.toLowerCase());
    return matchesCat && matchesSearch;
  });

  const categories = ['Hepsi', ...Object.values(Category)];
    ...
    <div className="pt-32 pb-24 min-h-screen">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="text-center mb-16">
          <h1 className="text-4xl md:text-6xl font-bold text-stone-900 mb-6 uppercase tracking-tighter">Koleksiyonlarımız</h1>
          <p className="text-stone-500 max-w-2xl mx-auto italic">Zarafet, konfor ve fonksiyonelliği yaşam alanlarınıza taşıyan modern mobilya tasarımlarımız.</p>
        </div>
          ...
          {filteredProducts.map((product) => (
            <div key={product.id} className="group">
              ...
              <div className="cursor-pointer text-center" onClick={() => navigate(`/product/${product.id}`)}>
                <h3 className="text-lg font-bold text-stone-900 mb-1 hover:text-amber-800 transition-colors uppercase tracking-tight">{product.name}</h3>
                <p className="text-stone-400 text-[10px] uppercase tracking-widest line-clamp-1">{product.description}</p>
              </div>
            </div>
          ))}
        </div>
        ...
        {filteredProducts.length === 0 && (
          <div className="text-center py-20">
            <p className="text-stone-400 text-xl italic font-light">Aradığınız kriterlere uygun ürün bulunamadı.</p>
          </div>
        )}
      </div>
    </div>
